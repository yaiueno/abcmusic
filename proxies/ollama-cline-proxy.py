#!/usr/bin/env python3
"""Ollama proxy for Cline: inject think=true/false per port."""
...
# "false" = fast (no thinking), "true" = thinking ON
THINK = os.environ.get("OLLAMA_PROXY_THINK", "false").lower() in ("1", "true", "yes")
...
class ProxyHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, fmt: str, *args) -> None:
        ...
        sys.stderr.write(f"[ollama-proxy:{mode}] " + (fmt % args) + "\n")

    def _forward(self, method: str) -> None:
        length = int(self.headers.get("Content-Length", 0))
        body = self.rfile.read(length) if length else b""
        body = inject_think(body, self.path)
...
def main() -> None:
    mode = "thinking ON" if THINK else "thinking OFF (fast)"
    server = ThreadingHTTPServer((LISTEN_HOST, LISTEN_PORT), ProxyHandler)
    url = f"http://{LISTEN_HOST}:{LISTEN_PORT}"
    print(f"Ollama proxy [{mode}]: {url} -> {UPSTREAM}", flush=True)
    print(f"Cline Base URL: {url}", flush=True)
    try:
